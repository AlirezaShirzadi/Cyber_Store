...
export default function AllItems({ allShopItems }: any) {
    ...
    return (
        <>
            ...
            <div className={`grid grid-cols-12 gap-x-5 ${shopItemViwes === 'items' ? 'gap-y-5 lg:gap-y-20' : 'gap-y-5'}`}>
                {allShopItems?.results?.map((item: any) => {
                    return <ShopItem item={item} view={shopItemViwes} />;
                })}
            </div>
        </>
    );
