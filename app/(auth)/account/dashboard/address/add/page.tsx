...
import { AddAddress } from "@/services/Account/service";
...
type AddressFormValues = {
    title: string;
    phone_number: string;
    province: number | string; // keep as string in select then cast to number on submit
    city: number | string;
    address: string;
    postal_code: string;
    is_default: boolean;
...
export default function Page() {
    ...
    useEffect(() => {
        ...
        async function loadProvinces() {
            ...
            try {
                ...
                // The API returns [[{ id, name }, ...]] according to issue description
    ...
    const onSubmit = async (form: AddressFormValues) => {
        ...
        try {
            ...
            if (res) {
                // Navigate to addresses list on success
                router.push("/account/dashboard/address");
            }
        } catch (e) {
            // Could add error UI; keeping minimal per requirements
            console.error(e);
        } finally {
            setSubmitting(false);
        }
    ...
    return (
        <div className="bg-[#E1E4FA] min-h-dvh pb-12">
            <Container>
                <div
                    className={`pt-[121px] pb-[27px] text-secondary text-3xl lg:text-5xl font-bold`}
                >
                ...
                <form
                    ...
                    className="bg-[#D2D5EE] rounded-[14px] p-6 grid grid-cols-1 gap-4 max-w-2xl mx-auto"
                >
                    ...
                    <div className="grid gap-1">
                        ...
                        <input
                            ...
                            {...register("phone_number", {
                                ...
                                pattern: {
                                    value: /^09[0-9]{9}$/,
                                    message: "شماره تلفن نامعتبر هست",
                                },
                            })}
                            placeholder="0912*******"
                        />
