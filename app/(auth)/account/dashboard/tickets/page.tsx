...
import {GetTicketsList} from "@/services/Tickets/service";
import {ToastContainer} from "react-toastify";
import TicketsListItem from "@/components/Dashboard/Tickets/TicketsListItem";
...
export default function Page() {
    ...
    return <div className="bg-[#E1E4FA] min-h-dvh">
        <Container className="grid grid-cols-12 gap-x-6 items-center min-h-dvh py-24 lg:py-0">
            ...
            <div className="col-span-12 lg:col-span-9 mt-8 lg:mt-0 space-y-[28px] max-w-4xl mx-auto w-full">
                <div className={`text-secondary text-3xl lg:text-5xl font-bold`}>تیکت های پشتیبانی</div>
                <div>
                    <Link className={`inline-block px-3.5 py-[7px] bg-primary rounded-[7px] text-white`}
                          href={`/account/dashboard/tickets/add`}>
                        اضافه کردن تیکت جدید
                    </Link>
                </div>
                <div className={`space-y-[13px]`}>
                    <div className={`text-secondary text-2xl font-extrabold`}>لیست تیکت‌ها</div>
                    <div className={`flex flex-col gap-3`}>
                        {loading && <div className="text-primary">در حال بارگذاری...</div>}
                        {!loading && tickets.length === 0 && (
                            <div className="text-primary">تیکتی یافت نشد.</div>
                        )}
                        {!loading && tickets.length > 0 && (
